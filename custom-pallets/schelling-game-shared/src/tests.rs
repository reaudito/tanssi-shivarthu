use crate::{
	mock::*,
	types::{Period, PhaseData, RangePoint, SchellingGameType, WonLost},
	Error, Event,
};
...
fn challenger_win_value_test() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(result.unwrap(), WonLost::Won);
		...
		assert_eq!(result.unwrap(), WonLost::Lost);
...
fn score_schelling_game_value_test() {
	new_test_ext().execute_with(|| {
		...
		let mean_values = TemplateModule::new_mean_reveal_score(key.clone());
		assert_eq!(2000, mean_values.unwrap());
		let result = TemplateModule::get_score_value_schelling(key.clone(), 4, RangePoint::ZeroToTen);
		assert_eq!(result.unwrap(), WonLost::Won);
		let result = TemplateModule::get_score_value_schelling(key.clone(), 7, RangePoint::ZeroToTen);
		assert_eq!(result.unwrap(), WonLost::Won);
		let result = TemplateModule::get_score_value_schelling(key.clone(), 13, RangePoint::ZeroToTen);
		assert_eq!(result.unwrap(), WonLost::Lost);
		let result = TemplateModule::get_score_value_schelling(key.clone(), 14, RangePoint::ZeroToTen);
		assert_eq!(result.unwrap(), WonLost::Won);
		let result = TemplateModule::get_score_value_schelling(key.clone(), 15, RangePoint::ZeroToTen);
		assert_eq!(result.unwrap(), WonLost::Lost);
	});
}
